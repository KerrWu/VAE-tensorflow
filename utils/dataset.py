...
class Dataset(object):
    def __init__(self, opts):
        ...
        print('trian images shape = ', imgs['data'].shape)
    ...
    def get_batch(self, tfrecord_file_name, shuffle=True):
        ...
        image = tf.reshape(row_image, [self.opts.IMG_H, self, self.opts.IMG_W, 3])
    ...
    # Read the image file.
        filename = self.opts.ROOT_DIR + image_name + '.jpg'
    ...
    def _get_output_filename(self, name, idx):
        return '%s/%s_%03d.tfrecord' % (self.opts.TFRECORD_DIR, name, idx)
