...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 17 12:32:37 2018
...
class VAE(object):
    ...
    def decoder(self, z):
        ...
        with tf.variable_scope("decoder"):
            ...
            dconv_in = tf.reshape(full1, [-1, 2, 2, dims * (32)])
...
#                tf.clip_by_value(1e-8 + 1 - gen_flat, 1e-10, 1)), 1)
        regulizer = tf.contrib.layers.l2_regularizer(0.05, scope=None)
        reg_term = tf.contrib.layers.apply_regularization(regulizer,weights_list=tf.trainable_variables ())


        dl = self.opts.G_LAMDA * tf.reduce_mean(decoder_loss) + reg_term
    ...
    def get_loss(self, x, scope, reuse_variables=None):
        ...
        with tf.variable_scope(tf.get_variable_scope(), reuse=reuse_variables):
            ...
            print("get l1 and l2")
            l1 = self.loss(mean, x, generated_imgs)
            print("Done")
            ...
            return l1, total_loss, generated_imgs
    ...
    def main(self):
        with tf.Graph().as_default(), tf.device('/cpu:0'):
                ...
                for i in range(self.opts.NUM_GPU):
                    with tf.device('/gpu:%d' % i):
                        with tf.name_scope('GPU_%d' % i) as scope:
                            ...
                            decode_loss, cur_loss, generated_images = self.get_loss(image_batch, scope=scope)
            ...
            with tf.Session(config=tf.ConfigProto(
                    ...
                    log_device_placement=True)) as self.sess:

                if restore == False:
                    print("initialie vriables ... ... ")
                    self.sess.run(init)
                else:
                    latest_ckpt = tf.train.latest_checkpoint(self.opts.CKPT_DIR)
                    print("restore variables from {name}".format(name=latest_ckpt))
                    ...
                    try:
                        ...
                        if not coord.should_stop():
                            ...
                            for batch_num in range(num_batches_per_epoch):
                                ...
                                _, lr_value, dl_value, loss_value, generated_images_value = self.sess.run([train_op, lr, decode_loss, 
                                                                                                                     cur_loss, generated_images])
    ...
    def cluster(self, image_dir):
        ...
        images_tensor = tf.convert_to_tensor(digits["images"],dtype=tf.float32)
        digits["data"] = self.test(images_tensor)
...
#        print("begin to creat dataset example")
#        n_img_per_row = 20
...
#                iy = 10 * j + 1
#                img[ix:ix + 8, iy:iy + 8] = X[i * n_img_per_row + j].reshape(self.opts.ENCODE_VECTOR_SIZE)
#
#        plt.imshow(img, cmap=plt.cm.binary)
#        plt.xticks([])
#        plt.yticks([])
#        plt.title('A selection from dataset')
